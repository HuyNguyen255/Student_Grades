...
SELECT 
	Department,
    CONCAT(ROUND(Number_student/(SELECT COUNT(*) FROM students_grade)*100,2),'%') '%Contribution',
...
-- Calculate standard deviation of total score
WITH avg_value as
(SELECT Avg(Total_score) Avg_score FROM Students_grade)
--
SELECT 
	POWER(SUM(Denorminator) / (SELECT COUNT(*) FROM Students_grade),0.5) Standard_deviation
FROM 
	(SELECT 
		Student_id,
		Total_score - (SELECT Avg_score FROM avg_value) value,
		POWER((Total_score - (SELECT Avg_score FROM avg_value)),2) Denorminator
	FROM Students_grade) as table1;
  --
SELECT std(total_score) std_total_score FROM students_grade;  

-- Calculate standard deviation of study hour
WITH avg_value as
(SELECT Avg(Study_Hours_per_Week) Avg_study_hour FROM Students_grade)
--
SELECT 
	POWER(SUM(Denorminator) / (SELECT COUNT(*) FROM Students_grade),0.5) Standard_deviation
FROM 
	(SELECT 
		Student_id,
		Study_Hours_per_Week - (SELECT Avg_study_hour FROM avg_value) value,
		POWER((Study_Hours_per_Week - (SELECT Avg_study_hour FROM avg_value)),2) Denorminator
	FROM Students_grade) as table1;
--
SELECT std(Study_Hours_per_Week) std_study_hour FROM students_grade

-- Calculate standard deviation of sleep hour
WITH avg_value as
(SELECT Avg(Sleep_Hours_per_Night) Avg_sleep_hour FROM Students_grade)
--
SELECT 
	POWER(SUM(Denorminator) / (SELECT COUNT(*) FROM Students_grade),0.5) Standard_deviation
FROM 
	(SELECT 
		Student_id,
		Sleep_Hours_per_Night - (SELECT Avg_sleep_hour FROM avg_value) value,
		POWER((Sleep_Hours_per_Night - (SELECT Avg_sleep_hour FROM avg_value)),2) Denorminator
	FROM Students_grade) as table1;
--
SELECT std(Sleep_Hours_per_Night) std_sleep_hour FROM students_grade
